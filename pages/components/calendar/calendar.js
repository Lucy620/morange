Component({
  ...
  methods: {
    ...
    bindDateChange: function(e) {
      ...
      this.display(parseInt(arr[0]), parseInt(arr[1]), 1)
    },
    ...
    createDays: function(year, month) {
       ...
       * 显示所存在的日程
       */
      let defaultMonth = this.data.defaultMonth
      for (let item of thisMonthDays) {
        for (let val of defaultMonth) {
          if (val == item.year + '-' + item.monthFormat + '-' + item.dateFormat) {
            item.self = true
          }
        }
      }
    ...
    createEmptyGrids: function(year, month) {
      let week = new Date(Date.UTC(year, month - 1, 0)).getDay(),
        empytGridsBefore = [],
        empytGridsAfter = [],
